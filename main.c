...
/*
03B6FD1C  00 02 00 08 00 54 72 65 65 54 68 6F 72 B7 FC 10  ...TreeThor·ü
03B6FD2C  00 00 00 00 00 96 66 07 00 31 30 30 34 33 39 64  .....–f.100439d
03B6FD3C  06 00 37 32 31 2E 30 30 [DE 87 AD 2A] 00 00 00 00  .721.00Þ‡­*....
      ...
      if(init_connect(&GC) == 1)
      {
       ...
       if(GV.bytes_recv != -1 && GV.bytes_recv != 0)
       {
        ...
        else
        {
         ...
         CopyStringPos((unsigned char*)SC.OffsetPrimary, (unsigned char*)GV.packet_decrypt, sizeof(SC.OffsetPrimary)-1, 0, 13);
         ...
         if(!CmpStringPos((unsigned char*)GV.packet_decrypt, (unsigned char*)login_failed, sizeof(login_failed)-1, 18, 0))
         {
          ...
          if(init_connect(&GC) == 1)
          {
           ...
           if(SendServerLogin(&GC, &SC, &GV))
           {
            ...
            #ifdef _WIN32
            CThreadRecvPacket = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ThreadRecvPacket, (LPVOID)0, 0, 0);
            CThreadSendTimePacket = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ThreadSendTimePacket, (LPVOID)0, 0, PThread);
            #elif LINUX
            ...
            */
            ...
            if(GetStatusRecvPacket() == TRUE)
             SelectChannelLogin(&GC, &SC, &GV);            // SELEÇÂO DE CANAL DO SERVIDOR
            
            BarSpace();
            TextCentralize((const char*)"DETHONBOT CARREGADO COM SUCESSO");
            BarSpace();
            
            if(SC.TYPE_SCRIPT == 0x01)
             CThreadMasterPlayer = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)MasterPlayerGame, (LPVOID)0, 0, PThread);
            else if(SC.TYPE_SCRIPT == 0x00)
             CThreadCommonPlayer = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)CommomPlayerGame, (LPVOID)0, 0, PThread);
            else if(SC.TYPE_SCRIPT == 0x02)
             CThreadPractice = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)InitPractice, (LPVOID)0, 0, PThread);
           }
          }
          /*else
          ...
          for(;;)
          {
           ...
           if(SC.TYPE_SCRIPT == 3)
           {
            if((GetStatusDialog() == FALSE) && (GetStatusConsole() == FALSE) && (GetStatusRecvPacket() == TRUE))
            {
             SetStatusConsole(TRUE);
             #ifdef _WIN32
              CThreadConsole = (HWND)CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ThreadConsole, (LPVOID)0, 0, PThread);
             #elif LINUX
          ...
          if((SC.TYPE_SCRIPT == 0x00) || (SC.TYPE_SCRIPT == 0x05) || (SC.TYPE_SCRIPT == 0x06) || (SC.TYPE_SCRIPT == 0x07))
          {
           IDRoomConfirmed(0);
           SC.TYPE_SCRIPT = 0x00;
           SC.P_ROOM_ID = 0x00;
...
{
 #ifdef _WIN32
  hWndDialog = (HWND)DialogBoxParam(GetModuleHandle(0), (LPCTSTR)MAKEINTRESOURCE(IDD_DIALOG1), 0, (DLGPROC)GoToProc, 0);
  ExitThread(0);
 #elif LINUX
  // GTK LINUX
 #endif
...
{ 
 ...
 /*if(((GV.packet_decrypt[9] == 0x5D) && (GV.packet_decrypt[11] == 0xBA)))
  SetMyTurn(0); // Erro, não é meu turno
 */
 ...
 */
 ...
 if((buffer_tmp[0] == 0x01) && (buffer_tmp[1] == 0x00) && (buffer_tmp[2] == 0x00) && ((unsigned char)SC.OFFSET_CHAR[0] == (unsigned char)0x00))
 {
  if(((buffer_tmp[4] == 0x00)))
  {
   ...
   printf("OFFSET CHAR [%02x] [%02x] [%02x] [%02x]\n", (unsigned char)SC.OFFSET_CHAR[0], (unsigned char)SC.OFFSET_CHAR[1], (unsigned char)SC.OFFSET_CHAR[2], (unsigned char)SC.OFFSET_CHAR[3]);
   GetNick(&GV);
 }
 }
 ...
 { 
  ...
  tmp = 0;
  for(tmp; tmp < GV.bytes_recv_piece; tmp++)
  {
