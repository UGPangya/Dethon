 ...
 if(!GC->AutoLogin)
 {
  for(;;)
  {
   for(int i = 0; i < MAX_SERVER_LOGIN_CONFIG-1; i++)
   {
    ...
    printf("NAME [%s] - OFFSET [%d]\n", SC->SUBSERVERNAME[SC->server_game_change][i], SC->OFFSETSUBSERVER[SC->server_game_change][i]);
   }
...
void GameDecryptFunc(struct GlobalVariables *GV)
{
 ...
 if(retn_tmp != 0 && retn_tmp != -1)
 {
  for(;;)
  {    
   if((retn_tmp >= PACKET_LIMITED) && PACKET_LIMITED != -1)
    break;
     
   tmp_ebp = ((unsigned char)GV->PacketRecv[tmp_other+1]*0x100)+(unsigned char)GV->PacketRecv[tmp_other]+3;
   ...
   if(tmp_other+((unsigned char)GV->PacketRecv[tmp_other+1]*0x100)+(unsigned char)GV->PacketRecv[tmp_other]+3 <= retn_tmp-1)
   {
    if(RECV_DEBUG_MODE_TYPE() == 1)
     printf("Info Next -> %x Addr %x\n", ((unsigned char)GV->PacketRecv[tmp_other+1]*0x100)+(unsigned char)GV->PacketRecv[tmp_other]+3, tmp_other+((unsigned char)GV->PacketRecv[tmp_other+1]*0x100)+(unsigned char)GV->PacketRecv[tmp_other]+3);
    tmp_other += ((unsigned char)GV->PacketRecv[tmp_other+1]*0x100)+(unsigned char)GV->PacketRecv[tmp_other]+3;
   }
   else
    break;
  }
...
void ReloadGame()
{
 if((GpSC->TYPE_SCRIPT == 0x04) || (GpSC->TYPE_SCRIPT == 0x01))
 {
  ...
  Sleep(500);
  create_room(GpSC->ROOM_NAME, GpSC->NUMBER_PLAYER, GpSC->NUMBER_HOLE, GpSC->TYPE_MAP, GpSC->TIME_MIN, GpSC->ROOM_PASSWORD);
