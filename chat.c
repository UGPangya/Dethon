	...
	if ((aux == 0x00) || (aux == 0x01))
	{
		if (CmpString("ROOM_SEND ID", buffer_tmp + tmp + 2, sizeof("ROOM_SEND ID") - 1))
		{
			if (SC->TYPE_SCRIPT == 0x00)
				sscanf_s(buffer_tmp + tmp + 2, "ROOM_SEND ID [%d] PASSWORD [%s]", &SC->P_ROOM_ID, SC->P_PASSWORD_ROOM,
				         _countof(SC->P_PASSWORD_ROOM));
		}
	}
...
void SendChatMsg(struct GlobalVariables* GV, char* msg_send)
{
	...
	send_msg_part1[1] = (char)(strlen(msg_send) + sizeof(send_msg_part2) + strlen(GV->NickName) + sizeof(send_msg_part1) - 6);
	send_msg_part1[7] = (char)strlen(GV->NickName);
	send_msg_part2[0] = (char)strlen(msg_send);
	...
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)GV->NickName, strlen(GV->NickName),
	              sizeof(send_msg_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)send_msg_part2, sizeof(send_msg_part2) - 1,
	              strlen(GV->NickName) + sizeof(send_msg_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)msg_send, strlen(msg_send),
	              sizeof(send_msg_part2) + strlen(GV->NickName) + sizeof(send_msg_part1) - 2, 0);
	EncryptSendPacket((char*)buffer_tmp,
	                  strlen(msg_send) + sizeof(send_msg_part2) + strlen(GV->NickName) + sizeof(send_msg_part1) - 2, 0);
	sprintf_s(GV->CInfo, _countof(GV->CInfo), "SIZE [%d] Send Packet",
	          strlen(msg_send) + sizeof(send_msg_part2) + strlen(GV->NickName) + sizeof(send_msg_part1) - 3);
...
void SendPMMsg(struct GlobalVariables* GV, char* msg_send, char* player)
{
	char buffer_tmp[127];
	send_msg_part1[0] = rand() % 0xFE + 0x01;
	send_msg_part1[1] = (char)(strlen(msg_send) + sizeof(send_msg_part2) + strlen(player) + sizeof(send_msg_part1) - 6);

	send_msg_part1[5] = 0x2A;
	send_msg_part1[7] = (char)strlen(player);
	send_msg_part2[0] = (char)strlen(msg_send);
	PacketSoma((unsigned char*)send_msg_part1, sizeof(send_msg_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)send_msg_part1, sizeof(send_msg_part1) - 1, 0, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)player, strlen(player), sizeof(send_msg_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)send_msg_part2, sizeof(send_msg_part2) - 1,
	              strlen(player) + sizeof(send_msg_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, (unsigned char*)msg_send, strlen(msg_send),
	              sizeof(send_msg_part2) + strlen(player) + sizeof(send_msg_part1) - 2, 0);
	EncryptSendPacket((char*)buffer_tmp,
	                  strlen(msg_send) + sizeof(send_msg_part2) + strlen(player) + sizeof(send_msg_part1) - 2, 0);
	sprintf_s(GV->CInfo, _countof(GV->CInfo), "SIZE [%d] Send Packet",
	          strlen(msg_send) + sizeof(send_msg_part2) + strlen(player) + sizeof(send_msg_part1) - 3);
	AddStringToListBoxDebug(GV->CInfo);
}
