...
/*
[03] Número de players
[07] Size of do nome da sala 
[12] Números de hole 0x12 = 18
[40] Time 40 minutos
[00] ID Mapa
...
// Byte 0 = sizeof password (se não tiver password fica em zero e junta com a parte 3)
...
void create_room(char *room_name, int player_num, int hole_num, int type_map, unsigned int time_min, char *password)
{
 ...
 packet_create_room_part1[25] = strlen(room_name);
 packet_create_room_part1[1] = sizeof(packet_create_room_part3)+sizeof(packet_create_room_part2)+strlen(room_name)+strlen(password)+sizeof(packet_create_room_part1)-3-4;
 ...
 packet_create_room_part2[0] = strlen(password);
 
 CopyStringPos(_buffer_tmp, packet_create_room_part2, sizeof(packet_create_room_part2)-1, strlen(room_name)+sizeof(packet_create_room_part1)-1, 0);
 
 if(password[0] == 0x00)
 {
  CopyStringPos(_buffer_tmp, packet_create_room_part3, sizeof(packet_create_room_part3)-1, sizeof(packet_create_room_part2)+strlen(room_name)+sizeof(packet_create_room_part1)-2, 0);
 }
 else
 {
  CopyStringPos(_buffer_tmp, (unsigned char*)password, strlen(password), sizeof(packet_create_room_part2)+strlen(room_name)+sizeof(packet_create_room_part1)-2, 0);
  CopyStringPos(_buffer_tmp, packet_create_room_part3, sizeof(packet_create_room_part3)-1, strlen(password)+sizeof(packet_create_room_part2)+strlen(room_name)+sizeof(packet_create_room_part1)-2, 0);
 }
 
 
 EncryptSendPacket((char*)_buffer_tmp, strlen(password)+sizeof(packet_create_room_part3)+sizeof(packet_create_room_part2)+strlen(room_name)+sizeof(packet_create_room_part1)-3, 0);
...
void enter_room(unsigned int ID_ROOM, char *password)
{
 ...
 packet_enter_room_part1[9] = strlen(password);
 packet_enter_room_part1[1] = strlen(password)+sizeof(packet_enter_room_part1)-1-4;
  ...
  while(GetStatusRecvPacket() == TRUE)
  {
   for(int i = 0; i < 3; i++)
   {
    if(RpSC->COMMON_CONFIRMED[i] == 0x01)
    {
     ...
     RpSC->COMMON_CONFIRMED[i] = 0x03;  // ESTÀGIO FINAL
     */
...
DWORD CommomPlayerGame(LPVOID Param)
{
 ...
 while(GetStatusRecvPacket() == TRUE && (RpSC->TYPE_SCRIPT != 0x07))
 {
  ...
  if(GetStatusRecvPacket() == TRUE)
  {
   ...
   while(RpSC->TYPE_SCRIPT == 0x05)     // Enquanto não receber o packet de confirmação de inicio do jogo, continua no while
   {
   ...
   if(RpSC->TYPE_SCRIPT == 0x06)
   {
    ...
    SetMyTurn(0);
    // Nesse momento o RpSC->TYPE_SCRIPT é jogado pra 0x06
    InitGameMap();
