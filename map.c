...
unsigned char packet_loading_map_part2[] =
	"\xB4\x14\x00\x73\xBA\x0C\x00\x07\x33\xCD\x0E\x00\x00\x00\x00\x00\x59\x00\x43\x03\x02\x00\x00\x14";
...
unsigned char packet_loading_map_part5[] =
	"\x4C\x1D\x00\x75\x06\x1A\x00\x01\x00\x00\x00\x00\x70\x0E\x00\x00\x04\xB8\x5E\x6E\xC3\x85\xEB\x38\xC4\x52\x38\x84\x42\x0A\x97\x12\x44";
...
unsigned char packet_exit_game[] =
	"\x29\x16\x00\xE5\x5C\x0F\x00\x01\xEE\xFF\x00\x00\x11\x00\x00\x00\x11\x00\x00\x00\x11\x00\x00\x00\x11\x00";
...
void InitGameMap()
{
	...
	//time_count = 5;
	while (((MpSC->TYPE_SCRIPT == 0x04) || (MpSC->TYPE_SCRIPT == 0x06) || (MpSC->TYPE_SCRIPT == 0x02)) && ((
		GetStatusRecvPacket() == TRUE) && ((GetPlayersConfirmed() >= 1) || (MpSC->TYPE_SCRIPT == 0x06))))
	{
		// ... continua at� acabar o jogo
...
// [\xFA\x43]
unsigned char packet_shot_part2[] = "\x64\x08\x00\x47\x97\x14\x00\x02\x00\x00\xFA\x43"; // Inicia For�a do Shot
...
//0E9750D8           13 BC D3 3E 24 12 74 E9 8B 61 AB BA A4     ��>$t�a���
//0E9750E8  41 B8 88 D9                                      A���
...
// Fecha o Game do Player advers�rio ...
...
unsigned char packet_shot_part4_shot_normal[] =
	"";
unsigned char packet_shot_part4_shot_pangya[] =
	"";
...
/*
0013F548   99 00 00 00 C2 2E 2C C3 F7 27 43 C1 CA B7 2D C2  �...�.,��'C�ʷ-�
0013F558   02 00 00 55 00 00 00 00 00 00 00 00 00 00 00 00  ..U............
0013F568   08 10 00 C7                                      .�
*/
...
unsigned char packet_shot_geral_part2[] =
	"\x0C\x00\x00\x00\x22\x91\xF7\x41\x6B\xCD\x4A\xC1\xAE\x5C\xDB\x43\x04\x00\x00\xE8\x03\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x08\x80\x00\x45\x00";
//\x00\x12
...
void Confirmed_Shot()
{
	...
	*/
	...
	CopyStringPos((unsigned char*)packet_shot_geral_part2_tmp, packet_shot_geral_part2, sizeof(packet_shot_geral_part2),
	              0, 0);
	...
	CopyStringPos((unsigned char*)buffer_tmp, packet_shot_geral_part2_tmp, sizeof(packet_shot_geral_part2_tmp),
	              sizeof(packet_shot_geral_part1) - 1, 0);
	CopyStringPos((unsigned char*)buffer_tmp, packet_shot_geral_part3, sizeof(packet_shot_geral_part3),
	              sizeof(packet_shot_geral_part2_tmp) + sizeof(packet_shot_geral_part1) - 2, 0);

	EncryptSendPacket((char*)buffer_tmp,
	                  sizeof(packet_shot_geral_part1) + sizeof(packet_shot_geral_part2_tmp) + sizeof(
		                  packet_shot_geral_part3) - 3, 0);
}
...
unsigned char trade_map_packet03[] =
	"\x2C\x1D\x00\x60\x46\x1A\x00\x02\x00\x00\x00\x00\x82\x14\x00\x00\x03\x52\xB8\x31\x42\x48\x01\x91\xC3\x0A\x57\xA3\x42\x29\xDC\xDF\x43";
unsigned char trade_map_packet04[] = "\xD6\x03\x00\xCD\xB3\x11\x00";
unsigned char trade_map_packet05[] = "\x97\x03\x00\xCE\x26\x34\x00"; // init Map j� existe ...
