...
unsigned int DecryptRecvPacket(unsigned char* buffer_decrypt, unsigned char* buffer_recv, unsigned int size)
{
	...
	if (retn_tmp != 0 && retn_tmp != -1)
	{
		for (;;)
		{
			...
			if (tmp_other + ((unsigned char)buffer_recv[tmp_other + 1] * 0x100) + (unsigned char)buffer_recv[tmp_other]
				+ 3 <= retn_tmp - 1)
			{
				if (RECV_DEBUG_MODE_TYPE() == 1)
					printf("Info Next -> %x Addr %x\n",
					       ((unsigned char)buffer_recv[tmp_other + 1] * 0x100) + (unsigned char)buffer_recv[tmp_other] +
					       3, tmp_other + ((unsigned char)buffer_recv[tmp_other + 1] * 0x100) + (unsigned char)
					       buffer_recv[tmp_other] + 3);
				tmp_other += ((unsigned char)buffer_recv[tmp_other + 1] * 0x100) + (unsigned char)buffer_recv[tmp_other]
					+ 3;
			}
			else
				break;
